...
services:
  web:
    image: ${CPA_NEXUS_IMAGE:-zonghao/cpa-nexus:0.1.28-amd64}
...
```bash
zonghao/cpa-nexus:0.1.28-amd64
```

Use a specific image with Docker Compose:

```bash
CPA_NEXUS_IMAGE=zonghao/cpa-nexus:0.1.28-amd64 docker compose pull
CPA_NEXUS_IMAGE=zonghao/cpa-nexus:0.1.28-amd64 docker compose up -d
```
...
docker buildx build --platform linux/amd64 \
  -t zonghao/cpa-nexus:0.1.28-amd64 \
  --push .
```
